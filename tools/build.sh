...
#
# Requires paideia-as >= 0.21.0 (fb_font.pdx-style @include_bytes for
# assets/fonts/glyphs_8x16.bin, landed at paideia-as v0.20.1+).
#
# libpdx-font is a library repo: there is no `manifest.pdxproj` /
# `entry` point to link into an executable (matches the libpdx-argv /
# libpdx-elevate / libpdx-audit shape, not the rm / cp / mv tool
# shape). This script's only job is a per-file syntax/encode check
# that produces one build-out/<name>.o per src/*.pdx — the same
# compile-check discipline those sibling libraries use.
